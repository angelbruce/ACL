...
namespace ACL.business.mcp.local
{
    ...
    public class TodoTool
    {
        ...
        public static TodoItem TodoCreate(
                [Description("关联的流程ID")] string? flowId
                , [Description("关联的节点ID")] string? nodeId
        ...
        [McpTool, Description("获取指定流程的所有TODO任务")]
        public static TodoItem[] GetTodosByFlow(string flowId)
        {
            return TodoStore.Instance.GetTodoItemsByFlow(flowId);
        }

        [McpTool, Description("获取指定流程节点的所有TODO任务列表，包含详细状态")]
        public static TodoItem[] GetTodosByFlowNode(
           [Required][Description("关联的流程ID")] string flowId
            , [Required][Description("关联的流程节点ID")] string nodeId)
        {
            return TodoStore.Instance.GetTodoItemsByFlowNode(flowId, nodeId);
        }

        [McpTool, Description("将任务关联到流程节点")]
        public static TodoItem? TodoLinkToNode(
             [Required][Description("任务id")] string id
            , [Required][Description("关联的流程ID")] string flowId
            , [Required][Description("关联的流程节点ID")] string nodeId)
        {
        ...
        [McpTool, Description("获取流程中未完成的任务数")]
        public static int TodoGetPendingCountByFlow([Required][Description("关联的流程ID")] string flowId)
        {
            var items = TodoStore.Instance.GetTodoItemsByFlow(flowId);
            items = items.Where(x => x.Status == TodoStatus.pending).ToArray();
            return items.Length;
        }


        public static int TodoGetPendingCountByFlowNode(
            [Required][Description("关联的流程ID")] string flowId,
             [Required][Description("关联的流程节点ID")] string nodeId)
        {
            var items = TodoStore.Instance.GetTodoItemsByFlow(flowId);
